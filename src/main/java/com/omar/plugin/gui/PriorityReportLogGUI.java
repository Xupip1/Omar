...
public class PriorityReportLogGUI implements InventoryHolder {
    ...
    private final List<PriorityReport> allReports;
    private final int page;
    private final int totalPages;

    public PriorityReportLogGUI(ConfigManager configManager, List<PriorityReport> allReports, int page) {
        ...
        this.inventory = Bukkit.createInventory(this, GUI_SIZE, Component.text(
                ConfigManager.translateColorCodes(
                        configManager.getRawMessageWithoutPrefix("gui-imlog-title"))
        ));
    ...
    private void initializeItems() {
        ...
        for (int i = startIndex; i < endIndex; i++) {
            PriorityReport report = allReports.get(i);
    ...
    private ItemStack createReportItem(PriorityReport report) {
        ItemStack item = new ItemStack(Material.WITHER_SKELETON_SKULL, 1);
        ItemMeta meta = item.getItemMeta();

        if (meta != null) {
            meta.displayName(configManager.colorize(
                    "&#f38ba8&l重点目标 &8#&7" + report.getId()
            ));

            List<String> rawLore = configManager.getRawMessageList("gui-imlog-report-lore");
    ...
    private ItemStack createPreviousPageItem() {
        ...
        if (meta != null) {
            meta.displayName(configManager.colorize(
                    configManager.getRawMessageWithoutPrefix("gui-imlog-previous-page")
            ));
            item.setItemMeta(meta);
        }
        return item;
    ...
    private ItemStack createNextPageItem() {
        ...
        if (meta != null) {
            meta.displayName(configManager.colorize(
                    configManager.getRawMessageWithoutPrefix("gui-imlog-next-page")
            ));
            item.setItemMeta(meta);
        }
        return item;
    ...
    private ItemStack createPageInfoItem() {
        ...
        ItemMeta meta = item.getItemMeta();
        if (meta != null) {
            String pageInfo = configManager.getRawMessageWithoutPrefix("gui-imlog-page-info")
    ...
    public void handleClick(InventoryClickEvent event) {
        ...
        if (slot >= 0 && slot < REPORTS_PER_PAGE) {
            ...
            if (reportIndex < allReports.size()) {
                PriorityReport report = allReports.get(reportIndex);
    ...
    private void teleportToReport(Player player, PriorityReport report) {
    ...
    private void openNewPage(Player player, int newPage) {
        PriorityReportLogGUI newGui = new PriorityReportLogGUI(configManager, allReports, newPage);
    ...
    public static void open(Player player, ConfigManager configManager,
        ...
        List<PriorityReport> reports = reportManager.getPriorityReports();
        PriorityReportLogGUI gui = new PriorityReportLogGUI(configManager, reports, 0);
